        ...
        <li v-for="(v,k) in page.list">
            ...
            <div class="text_body">
              ...
              <div @click="router.push({path: '/base/goods', query: {sku_id: v.sku_id}})">
                <div class="info flex">
                  <div class="text nowrap">{{ v.properties_value+' > '+v.supplier_name }}</div>
...
.refresh_time{position: fixed; z-index: 99; width: 100%;}
.goods_total{margin-top: 40px;}
.null{height: calc(100% - 40px - 248px);}
</style>
...
// 公共
defineOptions({name: 'PurchaseStock'});
...
const form = ref({stime:'', etime:'', key:''});
const customTime = ref({active:'all', title:'全部', list:[
  { type: 'time', label: '自定义', stime: Time.Date('Y/m/d'), etime: Time.Date('Y/m/d') },
  { label: '今天', value: 'today' },
  { label: '昨天', value: 'yesterday' },
  { label: '近7天', value: 'week' },
  { label: '近3月', value: 'month' },
  { label: '全部', value: 'all' },
]});
...
const setTime = (value: string): void => {
  ...
  switch(value) {
    ...
    case 'week':
      ...
      break;
    case 'month':
      customTime.value.title = '近3月';
      form.value.stime = Time.Date('Y/m/d', Time.StrToTime('-3 month'));
      form.value.etime = Time.Date('Y/m/d');
      break;
    case 'all':
      customTime.value.title = '全部';
      form.value.stime = '';
      form.value.etime = '';
      break;
  }
